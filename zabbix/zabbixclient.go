...
func init() {
	...
	flag.StringVar(&ZabbixType, "type", "dd", "告警消息的目标类型,支持`txdx(腾讯云短信)、txdh(腾讯云电话)、alydx(阿里云短信)、alydh(阿里云电话)、hwdx(华为云短信)、bddx(百度云短信)、rlydh(荣联云电话)、7moordx（七陌短信）、7moordh（七陌电话）、dd(钉钉)、wx(微信)、fs(飞书)、tg(Telegram) `")
	flag.StringVar(&PrometheusAlertUrl, "d", "http://127.0.0.1:8080/zabbix", "`PrometheusAlert的地址`")
	flag.BoolVar(&help, "h", false, "显示帮助")
	flag.Usage = usage
}
func usage() {
	fmt.Fprintf(os.Stderr, `Version 1.3 If you need help contact 244217140@qq.com or visit https://github.com/feiyu563/PrometheusAlert
